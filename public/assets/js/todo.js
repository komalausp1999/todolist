function task_done(id){
    $.get('/done/'+id,function(data){
        if(data=="OK"){
            //if task added then add a class done 
            $("#"+id).addClass('done');
        }else{
            $("#"+id).removeclass('done');
        }
...
function delete_task(id){
    $.get('/delete/'+id,function(data){
        if(data=="OK"){
            //target holds the id which is passed and then hides the list slowly
            var target = $("#"+id);
...
//Onlick function where id is passed of a form and then the addtask from is shown
...
$('#add_task').submit(function(event){
    ...
    if(title){
        $.post('/add',{title:title}).done(function(data){
            $('#add_task').hide('slow');
            //appends the entered data(title) in a list
